...
class Client {
            ...
            switch (message.t) {
                case null: {
                    ...
                    if (this.ready_status !== ReadyStates.CONNECTED) {
                        ...
                        this.ws.send(
                            JSON.stringify(
                                new packets.GateWayOpen(this.token, this.config)
                            )
                        );
                        this.on.gateway();
                    } else {
                ...
                case "MESSAGE_CREATE": {
                    switch (message.d.type) {
                        ...
                        case 16: {
                            ...
                            this.on.guild_discovery_grace_period_initial_warning(
                                message.d
                            );
                            break;
                        }
                        case 17: {
                            ...
                            this.on.guild_discovery_grace_period_final_warning(
                                message.d
                            );
                            break;
                        }
                        ...
                        case 27: {
                            ...
                            this.on.guild_application_premium_subscription(
                                message.d
                            );
                            break;
                        }
    ...
    async send_interaction(
        ...
        const application = search_res.applications.filter(
            (a) => a.name === application_name
        )[0];
        const command = search_res.application_commands.filter(
            (c) => c.name === command_name
        )[0];

        if (application.id !== command.application_id) {
            throw new DiscordUserBotsError(
                "application and command belong to different applications"
            );
        }
    ...
    async set_custom_status(custom_status = CustomStatusOpts) {
        ...
        return await this.fetch_request(`users/@me/settings`, {
            body: {
                custom_status: new constructs.CustomStatus(custom_status)
                    .contents,
            },
            method: "PATCH",
        });
